"""Step 85B — first-half form historical validation."""
...
def validate_season(path: Path, season: int) -> dict[str, object]:
    ...
    week1_unknown = (
        bool(
            week1["home_first_half_form_known"].not_().all()
            and week1["away_first_half_form_known"].not_().all()
        )
        if week1.height
        else True
    )
    ...
    week2plus_both_known_rate = (
        float(
            (
                week2plus["home_first_half_form_known"]
                & week2plus["away_first_half_form_known"]
            ).mean()
        )
        if week2plus.height
        else 0.0
...
def evaluate(report: dict[str, object]) -> tuple[list[str], list[str]]:
    ...
    for season_key, result in report["seasons"].items():
        ...
        if not result["week1_all_unknown"]:
            failures.append(
                f"{season}: Week 1 first-half form should be unknown"
            )
...
def render_markdown(report: dict[str, object]) -> str:
    lines = [
        ...
        (
            ...
            "Off EPA cov | Def EPA cov | Volume cov |"
        ),
        "| ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    ...
    for season, result in report["seasons"].items():
        lines.append(
            ...
            f"{result['features']['first_half_off_epa_advantage']['coverage']:.1%} | "
            f"{result['features']['first_half_def_epa_advantage']['coverage']:.1%} | "
            f"{result['features']['first_half_play_volume_advantage']['coverage']:.1%} |"
        )
    ...
    lines.extend(
        [
            ...
            "",
            (
                "Step 85A uses first-half play-by-play from completed prior "
                "games only. Current-game first-half plays must not contribute "
                "to that game's pregame features."
            ),
            "",
            (
                "Week 1 is intentionally unknown. From Week 2 onward, prior-game "
                "first-half history is permitted."
            ),
            "",
            "## Gate",
            "",
    ...
    if report["failures"]:
        ...
        lines.append(
            "**PASS** — the first-half form family is technically researchable."
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=Path("data/curated/first_half_form_features"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    for season in args.seasons:
        path = (
            args.input_dir
            / f"first_half_form_features_{season}.parquet"
        )
        if not path.exists():
            raise FileNotFoundError(
                f"Missing Step 85A artifact for {season}: {path}"
            )
    ...
    json_path = args.output_dir / "first_half_form_validation_85b.json"
    md_path = args.output_dir / "first_half_form_validation_85b.md"
    ...
    print("=" * 100)
    print("PROJECT GRIDIRON — STEP 85B FIRST-HALF FORM HISTORICAL VALIDATION")
    print("=" * 100)
    ...
    for season, result in seasons.items():
        print(
            ...
            f"off={result['features']['first_half_off_epa_advantage']['coverage']:.1%}  "
            f"def={result['features']['first_half_def_epa_advantage']['coverage']:.1%}  "
            f"volume={result['features']['first_half_play_volume_advantage']['coverage']:.1%}"
        )
