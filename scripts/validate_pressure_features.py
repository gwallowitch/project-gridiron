"""Historical validation report for pressure feature artifacts."""
...
SAMPLE_COLUMNS = (
    "home_off_dropbacks",
    "away_off_dropbacks",
    "home_off_pressure_events",
    "away_off_pressure_events",
    "home_def_dropbacks_faced",
    "away_def_dropbacks_faced",
...
def artifact_path(project_root: Path, season: int) -> Path:
    return (
        ...
        / "curated"
        / "pressure_features"
        / f"pressure_features_{season}.parquet"
    )


def validate_season(path: Path, season: int) -> SeasonValidation:
    if not path.exists():
        raise FileNotFoundError(f"Missing pressure artifact: {path}")
    ...
    home_known = float(frame["home_pressure_known"].mean())
    away_known = float(frame["away_pressure_known"].mean())
...
def _validate_cross_season(results: list[SeasonValidation]) -> None:
    ...
    for result in results:
        if result.home_known < 0.90 or result.away_known < 0.90:
            raise ValueError(
                f"{result.season} pressure known coverage is below 90%."
            )
        ...
        history = result.sample_means["home_pressure_history_weeks"]
        if history is None or history < 5.0:
            raise ValueError(
                f"{result.season} pressure history depth is unexpectedly low."
            )

        dropbacks = result.sample_means["home_off_dropbacks"]
        if dropbacks is None or dropbacks < 50.0:
            raise ValueError(
                f"{result.season} offensive dropback sample depth is too low."
            )

        pressure_events = result.sample_means["home_off_pressure_events"]
        if pressure_events is None or pressure_events < 5.0:
            raise ValueError(
                f"{result.season} pressure event sample depth is too low."
            )
...
def format_report(results: list[SeasonValidation]) -> str:
    lines = [
        "=" * 96,
        "PROJECT GRIDIRON — PRESSURE HISTORICAL VALIDATION",
        "=" * 96,
    ]
