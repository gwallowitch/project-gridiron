"""Validate the Step 78H promotion decision from the research registry."""
...
ROBUSTNESS = Path("data/reports/research/step78g_robustness.json")
OUTPUT = Path("data/reports/research/step78h_promotion.json")
SEASONS = (2022, 2023, 2024, 2025)
BASELINE = "five_weight_v1_baseline"
LOCKED = "five_weight_v1_locked"
...
def main() -> int:
    ...
    if not ROBUSTNESS.exists():
        raise FileNotFoundError("Missing Step 78G robustness report.")

    robust = json.loads(ROBUSTNESS.read_text(encoding="utf-8"))
    if robust.get("status") != "PROVISIONAL_PASS":
        raise ValueError("Step 78G did not produce PROVISIONAL_PASS.")
    if robust.get("candidate") != "def_epa_trend_0525":
        raise ValueError("78G candidate is not def_epa_trend_0525.")
    ...
    if missing:
        raise ValueError(
            "Step 78H registry is missing required results: "
            + ", ".join(missing)
        )

    score_deltas = [
        locked[season]["selection_score"] - baseline[season]["selection_score"]
        for season in SEASONS
    ]
    accuracy_deltas = [
        locked[season]["winner_accuracy"] - baseline[season]["winner_accuracy"]
        for season in SEASONS
    ]
    ...
    summary = {
        ...
        "looso_improves_count": sum(delta < 0 for delta in leave_one_out.values()),
    }
    ...
    print("=" * 92)
    print("PROJECT GRIDIRON — STEP 78H FIVE-WEIGHT PROMOTION LOCK")
    print("=" * 92)
    print(f"Candidate................. {LOCKED}")
    print("def_epa_trend_weight...... 5.25")
    print(f"Mean score delta.......... {summary['mean_score_delta']:+.6f}")
    print(f"Mean accuracy delta....... {summary['mean_accuracy_delta']:+.3%}")
    print(
        "Season record............. "
        f"{summary['season_wins']}-{summary['season_losses']}-{summary['season_ties']}"
    )
    print(f"LOOSO improvements........ {summary['looso_improves_count']}/4")
