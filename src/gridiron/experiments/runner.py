...
def run_experiments(
    ...
    _validate_rest_input(experiments, rest_features)
    results = [
        _run_one(
            schedule=schedule,
            pgr=pgr,
...
def _run_one(
    *,
    ...
    if predictions["rest_advantage"].null_count():
        raise ValueError(
            "Rest features do not cover every prediction game."
        )
...
def _validate_rest_input(
    experiments: list[ExperimentConfig],
    rest_features: pl.DataFrame | None,
) -> None:
    needs_rest = any(
        experiment.rest_weight != 0.0
        for experiment in experiments
    )
    if needs_rest and rest_features is None:
        raise ValueError(
            "Rest features are required for non-zero rest weights."
        )
    if rest_features is None:
        return

    missing = _REQUIRED_REST_COLUMNS.difference(
        rest_features.columns
    )
    if missing:
