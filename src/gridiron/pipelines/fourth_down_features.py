"""Fourth-down efficiency feature pipeline."""
...
class FourthDownFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe fourth-down features."""
    ...
    def pipeline_name(self) -> str:
        return "Fourth Down Features Pipeline"

    @property
    def dataset_name(self) -> str:
        return "fourth_down_features"

    @property
    def expected_output_path(self) -> Path:
        return (
            self.paths.root / "data" / "curated" / "fourth_down_features"
            / f"fourth_down_features_{self.season}.parquet"
        )
    ...
    def execute(self) -> PipelineArtifact:
        ...
        features = build_fourth_down_features(schedule, pbp)

        self.set_stage("feature validation")
        validate_fourth_down_features(features)
...
def run_fourth_down_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
    ...
    """Run the fourth-down feature pipeline for one season."""
    return FourthDownFeaturesPipeline(
        season=season,
