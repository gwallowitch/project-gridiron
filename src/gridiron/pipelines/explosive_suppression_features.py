"""Explosive-play suppression feature pipeline."""
...
class ExplosiveSuppressionFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe explosive-play features."""
    ...
    def pipeline_name(self) -> str:
        return "Explosive Suppression Features Pipeline"

    @property
    def dataset_name(self) -> str:
        return "explosive_suppression_features"
    ...
    def expected_output_path(self) -> Path:
        return (
            ...
            / f"explosive_suppression_features_{self.season}.parquet"
        )
    ...
    def execute(self) -> PipelineArtifact:
        ...
        features = build_explosive_suppression_features(
            schedule,
            pbp,
        )

        self.set_stage("feature validation")
        validate_explosive_suppression_features(features)
...
def run_explosive_suppression_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
    ...
    """Run the explosive-suppression pipeline for one season."""
    return ExplosiveSuppressionFeaturesPipeline(
        season=season,
