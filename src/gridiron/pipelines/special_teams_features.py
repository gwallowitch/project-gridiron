"""Special-teams feature pipeline."""
...
class SpecialTeamsFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe special-teams features."""
    ...
    def pipeline_name(self) -> str:
        return "Special Teams Features Pipeline"

    @property
    def dataset_name(self) -> str:
        return "special_teams_features"
    ...
    def expected_output_path(self) -> Path:
        return (
            ...
            / f"special_teams_features_{self.season}.parquet"
        )
    ...
    def execute(self) -> PipelineArtifact:
        ...
        features = build_special_teams_features(schedule, pbp)

        self.set_stage("feature validation")
        validate_special_teams_features(features)
...
def run_special_teams_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
) -> PipelineRunResult:
    """Run the special-teams feature pipeline."""
    return SpecialTeamsFeaturesPipeline(
        season=season,
