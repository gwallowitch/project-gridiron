...
class ThirdDownFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe third-down features."""
    ...
    def pipeline_name(self) -> str:
        return "Third Down Features Pipeline"

    @property
    def dataset_name(self) -> str:
        return "third_down_features"
    ...
    def expected_output_path(self) -> Path:
        return (
            ...
            / "curated"
            / "third_down_features"
            / f"third_down_features_{self.season}.parquet"
        )
    ...
    def execute(self) -> PipelineArtifact:
        ...
        features = build_third_down_features(schedule, pbp)

        self.set_stage("feature validation")
        validate_third_down_features(features)
...
def run_third_down_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
) -> PipelineRunResult:
    """Run the third-down feature pipeline."""
    return ThirdDownFeaturesPipeline(
        season=season,
