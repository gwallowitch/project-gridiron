"""Neutral game-state efficiency feature pipeline."""
...
class NeutralStateFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe neutral-state efficiency features."""
    ...
    def pipeline_name(self) -> str:
        return "Neutral State Features Pipeline"

    @property
    def dataset_name(self) -> str:
        return "neutral_state_features"
    ...
    def expected_output_path(self) -> Path:
        return (
            ...
            / f"neutral_state_features_{self.season}.parquet"
        )
    ...
    def execute(self) -> PipelineArtifact:
        ...
        features = build_neutral_state_features(schedule, pbp)

        self.set_stage("feature validation")
        validate_neutral_state_features(features)
...
def run_neutral_state_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
) -> PipelineRunResult:
    """Run the neutral-state feature pipeline."""
    return NeutralStateFeaturesPipeline(
        season=season,
