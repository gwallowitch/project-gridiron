"""Drive-efficiency feature pipeline."""
...
class DriveEfficiencyFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe drive-efficiency features."""
    ...
    def pipeline_name(self) -> str:
        return "Drive Efficiency Features Pipeline"

    @property
    def dataset_name(self) -> str:
        return "drive_efficiency_features"
    ...
    def expected_output_path(self) -> Path:
        return (
            ...
            / f"drive_efficiency_features_{self.season}.parquet"
        )
    ...
    def execute(self) -> PipelineArtifact:
        ...
        features = build_drive_efficiency_features(schedule, pbp)

        self.set_stage("feature validation")
        validate_drive_efficiency_features(features)
...
def run_drive_efficiency_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
) -> PipelineRunResult:
    """Run the drive-efficiency feature pipeline."""
    return DriveEfficiencyFeaturesPipeline(
        season=season,
