"""Field-position and hidden-yards feature pipeline."""
...
class FieldPositionFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe field-position features."""
    ...
    def pipeline_name(self) -> str:
        return "Field Position Features Pipeline"

    @property
    def dataset_name(self) -> str:
        return "field_position_features"
    ...
    def expected_output_path(self) -> Path:
        return (
            ...
            / f"field_position_features_{self.season}.parquet"
        )
    ...
    def execute(self) -> PipelineArtifact:
        ...
        features = build_field_position_features(schedule, pbp)

        self.set_stage("feature validation")
        validate_field_position_features(features)
...
def run_field_position_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
) -> PipelineRunResult:
    """Run the field-position feature pipeline."""
    return FieldPositionFeaturesPipeline(
        season=season,
