"""Passing-efficiency feature pipeline."""
...
class PassingFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe passing features."""
    ...
    def pipeline_name(self) -> str:
        return "Passing Features Pipeline"
    ...
    def expected_output_path(self) -> Path:
        return (
            ...
            / "curated"
            / "passing_features"
            / f"passing_features_{self.season}.parquet"
        )

    def execute(self) -> PipelineArtifact:
        self.set_stage("input validation")
        schedule_path = self.paths.schedule_file(
            self.season
        )
        if not schedule_path.exists():
            raise FileNotFoundError(
                f"Schedule file does not exist: "
                f"{schedule_path}"
            )

        self.set_stage("loading")
        schedule = pl.read_parquet(schedule_path)
        pbp = self.gateway.play_by_play(
            [self.season]
        )

        self.set_stage("feature calculation")
        features = build_passing_features(
            schedule,
            pbp,
        )

        self.set_stage("feature validation")
        validate_passing_features(features)
...
def run_passing_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
) -> PipelineRunResult:
    """Run the passing-efficiency feature pipeline."""
    return PassingFeaturesPipeline(
        season=season,
