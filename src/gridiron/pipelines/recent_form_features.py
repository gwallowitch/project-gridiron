...
class RecentFormFeaturesPipeline(BasePipeline):
    """Build and persist leakage-safe recent-form features."""
    ...
    def pipeline_name(self) -> str:
        return "Recent Form Features Pipeline"

    @property
    def dataset_name(self) -> str:
        return "recent_form_features"
    ...
    def expected_output_path(self) -> Path:
        return (
            ...
            / f"recent_form_features_{self.season}.parquet"
        )
    ...
    def execute(self) -> PipelineArtifact:
        ...
        features = build_recent_form_features(schedule, pbp)

        self.set_stage("feature validation")
        validate_recent_form_features(features)
...
def run_recent_form_features_pipeline(
    season: int,
    *,
    project_root: Path | str = Path("."),
    database_path: Path | str | None = None,
    ...
    """Run the recent-form feature pipeline for one season."""
    return RecentFormFeaturesPipeline(
        season=season,
