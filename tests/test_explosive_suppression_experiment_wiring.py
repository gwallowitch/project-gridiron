...
def test_explosive_suppression_weights_default_to_zero() -> None:
...
def test_explosive_weights_must_be_finite() -> None:
    with pytest.raises(
        ValueError,
        match="explosive_suppression_weight",
    ):
        validate_experiments(
            [config(explosive_suppression_weight=math.inf)]
        )


def test_explosive_weights_must_not_be_negative() -> None:
    with pytest.raises(
        ValueError,
        match="chunk_suppression_weight",
    ):
        validate_experiments(
            [config(chunk_suppression_weight=-1.0)]
        )


def test_76c_preserves_four_weight_research_lock() -> None:
    x = config()
